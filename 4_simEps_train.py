...
from four_simEps_model import model1, model2, model3, model4
# from m_net_factory import net_factory
...
class Network(object):
    ...
    def _init_logger(self):
        ...
        log_dir = '/.../model_weights/NEU_seg/simEps_4model_10p/'
    ...
    def run(self):
        ...
        optimizer_4 = torch.optim.Adam(self.model4.parameters(), lr=base_lr)
        ...
        scheduler_4 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_4, mode="max", min_lr = 0.0000001, patience=50, verbose=True)
        ...
        for epoch in range(1, epochs):
            ...
            running_ce_loss_4 = 0.0
            running_dice_loss_4 = 0.0
            ...
            running_train_iou_4 = 0.0
            running_train_dice_4 = 0.0
            ...
            running_dice_loss_val_4 = 0.0
            running_ce_loss_val_4 = 0.0
            ...
            running_val_iou_4 = 0.0
            running_val_dice_4 = 0.0
            running_val_accuracy_4 = 0.0
            ...
            for iteration in range (1, iter_per_epoch): #(zip(train_loader, unlabeled_train_loader)):
                ...
                x4_4, _, _, _, f4_4, outputs_4, _, _, _ = self.model4(inputs_S1)
                ...
                outputs_4_soft = torch.softmax(outputs_4, dim=1)
                ...
                x4_4_un, _, _, _, f4_4_un, un_outputs_4, _, _, _ = self.model4(inputs_U) #perturbed input
                #Softmax output
                ...
                un_outputs_soft_4 = torch.softmax(un_outputs_4, dim=1)
                ...
                loss_ce_4 = ce_loss(outputs_4, labels_S1.long())
                ...
                loss_dice_4 = dice_loss(labels_S1.unsqueeze(1), outputs_4)
                ...
                model4_sup_loss =0.5*(loss_ce_4 + loss_dice_4) 
                
                sup_loss = model1_sup_loss + model2_sup_loss + model3_sup_loss + model4_sup_loss
                ...
                sim_loss_14 = 0.5*(sim_loss(outputs_1, outputs_4.detach()) + sim_loss(outputs_1.detach(), outputs_4)) 
                sim_loss_23 = 0.5*(sim_loss(outputs_2, outputs_3.detach()) + sim_loss(outputs_2.detach(), outputs_3))
                sim_loss_24 = 0.5*(sim_loss(outputs_2, outputs_4.detach()) + sim_loss(outputs_2.detach(), outputs_4)) 
                sim_loss_34 = 0.5*(sim_loss(outputs_3, outputs_4.detach()) + sim_loss(outputs_3.detach(), outputs_4))  
                ...
                sim_loss_un_14 = 0.5*(sim_loss(un_outputs_1, un_outputs_4.detach()) + sim_loss(un_outputs_1.detach(), un_outputs_4))
                sim_loss_un_23 = 0.5*(sim_loss(un_outputs_2, un_outputs_3.detach()) + sim_loss(un_outputs_2.detach(), un_outputs_3))
                sim_loss_un_24 = 0.5*(sim_loss(un_outputs_2, un_outputs_4.detach()) + sim_loss(un_outputs_2.detach(), un_outputs_4))
                sim_loss_un_34 = 0.5*(sim_loss(un_outputs_3, un_outputs_4.detach()) + sim_loss(un_outputs_3.detach(), un_outputs_4))
                
                inp_sim_loss = sim_loss_12 + sim_loss_13 + sim_loss_14 + sim_loss_23 \
                    + sim_loss_24 + sim_loss_34 + sim_loss_un_12 + sim_loss_un_13 + sim_loss_un_14 \
                        + sim_loss_un_23 + sim_loss_un_24 + sim_loss_un_34
                ...
                lbl_pseudo_m1 = torch.argmax(torch.max(torch.max(outputs_2_soft.detach(), outputs_3_soft.detach()), outputs_4_soft.detach()), dim=1, keepdim=False)
                lbl_pseudo_m2 = torch.argmax(torch.max(torch.max(outputs_1_soft.detach(), outputs_3_soft.detach()), outputs_4_soft.detach()), dim=1, keepdim=False)
                lbl_pseudo_m3 = torch.argmax(torch.max(torch.max(outputs_1_soft.detach(), outputs_2_soft.detach()), outputs_4_soft.detach()), dim=1, keepdim=False)
                lbl_pseudo_m4 = torch.argmax(torch.max(torch.max(outputs_1_soft.detach(), outputs_2_soft.detach()), outputs_3_soft.detach()), dim=1, keepdim=False)
                # lbl_pseudo_m1 = torch.argmax(torch.max(outputs_1_soft.detach(), outputs_2_soft.detach()), dim=1, keepdim=False)
                # lbl_pseudo_m2 = torch.argmax(torch.max(outputs_1_soft.detach(), outputs_3_soft.detach()), dim=1, keepdim=False)
                # lbl_pseudo_m3 = torch.argmax(torch.max(outputs_2_soft.detach(), outputs_3_soft.detach()), dim=1, keepdim=False)
                # lbl_pseudo_m4 = torch.argmax(torch.max(outputs_2_soft.detach(), outputs_3_soft.detach()), dim=1, keepdim=False)
                ...
                lbl_pseudo_supervision4 = 0.5*ce_loss(outputs_4, lbl_pseudo_m4) + 0.5*dice_loss(lbl_pseudo_m4.unsqueeze(1), outputs_4)
                

                cps_loss_labeled = lbl_pseudo_supervision1 +  lbl_pseudo_supervision2 \
                    + lbl_pseudo_supervision3 + lbl_pseudo_supervision4
                ...
                pseudo_m1 = torch.argmax(torch.max(torch.max(un_outputs_soft_2.detach(), un_outputs_soft_3.detach()), un_outputs_soft_4.detach()), dim=1, keepdim=False)
                pseudo_m2 = torch.argmax(torch.max(torch.max(un_outputs_soft_1.detach(), un_outputs_soft_3.detach()), un_outputs_soft_4.detach()), dim=1, keepdim=False)
                pseudo_m3 = torch.argmax(torch.max(torch.max(un_outputs_soft_1.detach(), un_outputs_soft_2.detach()), un_outputs_soft_4.detach()), dim=1, keepdim=False)
                pseudo_m4 = torch.argmax(torch.max(torch.max(un_outputs_soft_1.detach(), un_outputs_soft_2.detach()), un_outputs_soft_3.detach()), dim=1, keepdim=False)
                # pseudo_m4 = torch.argmax(torch.max(un_outputs_soft_2.detach(), un_outputs_soft_3.detach()), dim=1, keepdim=False)
                ...
                pseudo_supervision4 = 0.5*ce_loss(un_outputs_4, pseudo_m4) + 0.5*dice_loss(pseudo_m4.unsqueeze(1), un_outputs_4)
                

                cps_loss = pseudo_supervision1 +  pseudo_supervision2 + pseudo_supervision3 + pseudo_supervision4

                consistency_weight = get_current_consistency_weight(iter_num // 150) #Consistency weight multipliers


                # loss = sup_loss + consistency_weight * cps_loss + 200*inp_sim_loss
                loss = sup_loss + consistency_weight * cps_loss  + consistency_weight*cps_loss_labeled + 50*inp_sim_loss
                ...
                running_ce_loss_4 += loss_ce_4.item()
                running_dice_loss_4 += loss_dice_4.item()
                ...
                running_train_iou_4 += mIoU(outputs_4, labels_S1)
                running_train_dice_4 += mDice(outputs_4, labels_S1)
                ...
                for param_group in optimizer_4.param_groups:
                    lr_4 = param_group['lr'] #For plotting the learning rate change during the training process# for param_group in optimizer_2.param_groups:
            ...
            epoch_train_iou_4 = ( running_train_iou_4) / (iter_per_epoch)
            epoch_train_dice_4 = ( running_train_dice_4) / (iter_per_epoch)
            ...
            epoch_ce_loss_4 = (running_ce_loss_4) / (iter_per_epoch)
            epoch_dice_loss_4 = (running_dice_loss_4) / (iter_per_epoch)
            ...
            self.logger.info('Train IoU-4: {}'.format(epoch_train_iou_4))
            self.writer.add_scalar('Train/IoU-4', epoch_train_iou_4, epoch)
            self.logger.info('Train Dice-4: {}'.format(epoch_train_dice_4))
            self.writer.add_scalar('Train/Dice-4', epoch_train_dice_4, epoch)
            ...
            self.logger.info('Train ce-loss-4: {}'.format(epoch_ce_loss_4))
            self.writer.add_scalar('Train/CE-Loss-4', epoch_ce_loss_4, epoch)
            self.logger.info('Train dice-loss-4: {}'.format(epoch_dice_loss_4))
            self.writer.add_scalar('Train/Dice-Loss-4', epoch_dice_loss_4, epoch)
            ...
            self.writer.add_scalar('info/lr4', lr_4, epoch)
            ...
            for i, pack in enumerate(test_loader, start=1):
                with torch.no_grad():
                    ...
                    _, _, _, _, _, pred_4, _, _, _= self.model4(images)
                ...
                loss_ce_4 = ce_loss(pred_4, gts.long())
                loss_dice_4 = 1 - mDice(pred_4, gts)


                val_loss = 0.5 * (loss_dice_1 + loss_ce_1) + 0.5 * (loss_dice_2 + loss_ce_2) \
                    + 0.5 * (loss_dice_3 + loss_ce_3) + 0.5 * (loss_dice_4 + loss_ce_4)
                ...
                running_dice_loss_val_4 += loss_dice_4.item()
                running_ce_loss_val_4 += loss_ce_4.item()
                ...
                running_val_iou_4 += mIoU(pred_4, gts)
                running_val_dice_4 += mDice(pred_4, gts)
                running_val_accuracy_4 += pixel_accuracy(pred_4, gts)

                 
            epoch_loss_val = running_val_loss / len(test_loader)
            epoch_val_dice_loss_1 = running_dice_loss_val_1 / len(test_loader)
            epoch_val_ce_loss_1 = running_ce_loss_val_1 / len(test_loader)
            epoch_val_dice_loss_2 = running_dice_loss_val_2 / len(test_loader)
            epoch_val_ce_loss_2 = running_ce_loss_val_2 / len(test_loader)
            epoch_val_dice_loss_3 = running_dice_loss_val_3 / len(test_loader)
            epoch_val_ce_loss_3 = running_ce_loss_val_3 / len(test_loader)
            epoch_val_dice_loss_4 = running_dice_loss_val_4 / len(test_loader)
            epoch_val_ce_loss_4 = running_ce_loss_val_4 / len(test_loader)
            ...
            epoch_dice_val_3 = running_val_dice_3 / len(test_loader)
            epoch_iou_val_3 = running_val_iou_3 / len(test_loader)
            epoch_accuracy_val_3 = running_val_accuracy_3 / len(test_loader)

            epoch_dice_val_4 = running_val_dice_4 / len(test_loader)
            epoch_iou_val_4 = running_val_iou_4 / len(test_loader)
            epoch_accuracy_val_4 = running_val_accuracy_4 / len(test_loader)

            scheduler_1.step(epoch_dice_val_1)
            scheduler_2.step(epoch_dice_val_2)
            scheduler_3.step(epoch_dice_val_3)
            scheduler_4.step(epoch_dice_val_4)
            ...
            self.logger.info('Val dice_loss_4 : {}'.format(epoch_val_dice_loss_4))
            self.writer.add_scalar('Val/Dice-loss_4', epoch_val_dice_loss_4, epoch)
            self.logger.info('Val ce_loss_4 : {}'.format(epoch_val_ce_loss_4))
            self.writer.add_scalar('Val/ce-loss_4', epoch_val_ce_loss_4, epoch)
            ...
            self.logger.info('Val dice_4 : {}'.format(epoch_dice_val_3))
            self.writer.add_scalar('Val/DSC-4', epoch_dice_val_3, epoch)

            self.logger.info('Val IoU_4 : {}'.format(epoch_iou_val_4))
            self.writer.add_scalar('Val/IoU-4', epoch_iou_val_4, epoch)

            self.logger.info('Val Accuracy_4 : {}'.format(epoch_accuracy_val_4))
            self.writer.add_scalar('Val/Accuracy-4', epoch_accuracy_val_4, epoch)
            ...
            mdice_coeff_4 =  epoch_dice_val_4
            ...
            if self.best_dice_coeff_4 < mdice_coeff_4:
                self.best_dice_coeff_4 = mdice_coeff_4
                self.save_best_model_4 = True

                self.patience_4 = 0
            else:
                self.save_best_model_4 = False
                self.patience_4 += 1 
            ...
            if self.save_best_model_1:
                ...
                torch.save(state_1, Checkpoints_Path + '/simEps_10p_4m_4_1.pth')
            ...
            if self.save_best_model_2:
                ...
                # state["best_loss"] = self.best_loss
                torch.save(state_2, Checkpoints_Path + '/simEps_10p_4m_4_2.pth')
            ...
            if self.save_best_model_3:
                ...
                # state["best_loss"] = self.best_loss
                torch.save(state_3, Checkpoints_Path + '/simEps_10p_4m_4_3.pth')
            
            if self.save_best_model_4:
                state_4 = {
                "epoch": epoch,
                "best_dice_4": self.best_dice_coeff_4,
                "state_dict": self.model4.state_dict(),
                "optimizer": optimizer_4.state_dict(),
                }
                # state["best_loss"] = self.best_loss
                torch.save(state_4, Checkpoints_Path + '/simEps_10p_4m_4.pth')
            ...
            self.logger.info(
                'current best dice coef: model-1: {}, model-2: {}, model-3: {}, model-4: {}'.format(self.best_dice_coeff_1, self.best_dice_coeff_2, self.best_dice_coeff_3, self.best_dice_coeff_4))

            self.logger.info('current patience: m1: {}, m2: {}, m3: {},  m4: {}'.format(self.patience_1, self.patience_2, self.patience_3, self.patience_4))

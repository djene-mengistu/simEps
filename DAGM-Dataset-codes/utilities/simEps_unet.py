...
class Decoder_mcps(nn.Module):
    def __init__(self, params):
        super(Decoder_mcps, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
    ...
    def forward(self, feature, shape):
        ...
        x = self.up4(x, x0)
        f4 = x
        dp0_out_seg = self.out_conv(x) #Final output 
        return x4, f1, f2, f3, f4, dp0_out_seg, dp1_out_seg, dp2_out_seg, dp3_out_seg
...
class UNet_f(nn.Module):  #Normal Unet
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_mcps(params)

    def forward(self, x):
        shape = x.shape[2:]
        feature = self.encoder(x)
        x4, f1, f2, f3, f4, dp0_out_seg, dp1_out_seg, dp2_out_seg, dp3_out_seg = self.decoder(feature, shape)
        return x4, f1, f2, f3, f4, dp0_out_seg, dp1_out_seg, dp2_out_seg, dp3_out_seg
...
class UNet_g(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        params = {'in_chns': in_chns,
                  ...
                  'acti_func': 'relu'}
        self.encoder = Encoder(params)
        self.main_decoder = Decoder_mcps(params)
    ...
    def forward(self, x):
        shape = x.shape[2:]
        feature = self.encoder(x)
        feature_d = [FeatureDropout(i) for i in feature]
        x4, f1, f2, f3, f4, dp0_out_seg, dp1_out_seg, dp2_out_seg, dp3_out_seg = self.main_decoder(feature_d, shape)
...
class UNet_h(nn.Module):
    ...
    def forward(self, x):
        shape = x.shape[2:]
        feature = self.encoder(x)
        feature_n = [FeatureNoise()(i) for i in feature]
        x4, f1, f2, f3, f4, dp0_out_seg, dp1_out_seg, dp2_out_seg, dp3_out_seg = self.main_decoder(feature_n, shape)
        return x4, f1, f2, f3, f4, dp0_out_seg, dp1_out_seg, dp2_out_seg, dp3_out_seg
